...
void ServerImpl::Start(uint16_t port, uint32_t n_accept, uint32_t n_workers) {
    ...
    _logger->info("Start coroutine network service");
...
void ServerImpl::OnRun() {
    while (_running) {
        ...
        _engine.run([this, client_socket] { this->_WorkerFunction(client_socket); } );
    }
...
void ServerImpl::_WorkerFunction(int client_socket) {
    ...
    _logger->debug("On new connection");
