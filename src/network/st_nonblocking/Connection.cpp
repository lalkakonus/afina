...
Connection(int s, std::shared_ptr<Afina::Storage> pStorage, std::shared_ptr<spdlog::logger> logger) :
		   _logger(logger), _pStoragee(pStorage), _socket(s), _shift(0), _isActive(false) {
...
void Connection::DoRead() {
	try {
		while (((readed_bytes = read(client_socket, client_buffer + _shift, 
			...
			while (readed_bytes > 0) {
				...
				if (command_to_execute && arg_remains == 0) {
					...
					_event.events = EPOLLIN | EPOLOUT | EPOLLRDHUP | EPOLLPRI;
...
void Connection::DoWrite() {
	iovcnt = _response.size();
	iovec* answers = new iovec[iovcnt];
	for (int i = 0; i < iovcnt; i++) {
		answer[i].iov_base = _response[i].data();
		answer[i].iov_len = _response[i].size();
	}

	auto residue = writev(_socket, iovec, iovcnt);
	int pos;
	...
	if (residue) {
		_response[pos].erase(response[pos].begin(), response[pos].begin() + residue)
	}
